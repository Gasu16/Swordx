...
void getOccurrency(FILE *fileIN, FILE *fileOUT){
    char array[DIM];
    char words[DIM];
    int i, len, index, match;
    int count[DIM];
    
    while(fscanf(fileIN, "%s", array) != EOF){ // Leggi file in input
        if(ispunct(array[len-1])){
            array[len-1] = '\0';
        }
        match = 1; // C'e' corrispondenza
        for(i = 0; i < DIM; i++){
            if(strcmp(words[i], array)){
                match = 0;
            }
        }
        if(match){
            strcpy(words[i], array);
            count[index]++;
            index++;
        }
        else{
